...
const CatalogPage = () => {
  ...
  const {filter_products,
         sorting,
        filters: {text,category,area,city,mxPrice,mnPrice,price},
        all_products,
        updateFilterValue
      }=useFilterContext();
  ...
  // Price range slider
  const PriceRange = () => {
    const [minPrice, setMinPrice] = useState(categoryParam === 'sale' ? 90000 : {mnPrice})
    const [maxPrice, setMaxPrice] = useState(categoryParam === 'sale' ? 250000 : {mxPrice})
          ...
          <Col lg={8} xl={9} className='position-relative overflow-hidden pb-5 pt-4 px-3 px-xl-4 px-xxl-5'>
            ...
            <div className='d-flex flex-sm-row flex-column align-items-sm-center align-items-stretch my-2'>
              ...
              <div className='d-none d-sm-flex align-items-center flex-shrink-0 text-muted'>
                ...
                <span className='fs-sm mt-n1'>300 properties</span>
              </div>
            </div>

            {/* Catalog grid */}
            <Row xs={1} sm={2} xl={3} className='g-4 py-4'>
              {categoryParam === 'sale' ? filter_products.map((product,indx) => (
                <Col key={indx}>
                  <PropertyCard
