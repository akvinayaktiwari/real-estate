...
const BlogSinglePage = () => {
  ...
  //   return savedState !== null ? JSON.parse(savedState) : 'Initial State';
  // });
  ...
  useEffect(() => {
    // Set current URL after component mounts to ensure `window` is available
    ...
    >
      <Container className='mt-5 mb-md-4 py-5'>
        ...
        <Breadcrumb className='mb-3 pt-md-3'>
          ...
          <Breadcrumb.Item linkAs={Link} href='/blog'>Blogs</Breadcrumb.Item>
          <Breadcrumb.Item active>Blog</Breadcrumb.Item>
        </Breadcrumb>

        {/* Category + title */}
        <a className='nav-link d-inline-block fw-normal text-uppercase px-0 mb-2' href='#'>{post.category}</a>
        ...
        <div className='mb-4 pb-1'>
          <ul className='list-unstyled d-flex flex-wrap mb-0 text-nowrap'>
            <li className='me-3'>
              ...
              {post.createdDate ? post.createdDate.substring(0, 10) : 'N/A'}
            </li>
        ...
        <div className='mb-4 pb-md-3'>
          <ImageLoader src={post.image} width={1296} height={600} alt='Post image' className='rounded-3' />
        </div>
            ...
            <div className='sticky-top py-md-5 mt-md-5'>
              <div className='d-flex flex-md-column align-items-center my-2 mt-md-4 pt-md-5'>
                ...
                <OverlayTrigger placement='top' overlay={<Tooltip>Share with Facebook</Tooltip>}>
                  <Button href={`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(currentUrl)}`} size='xs' variant='icon btn-light-primary rounded-circle shadow-sm mb-md-2 me-md-0 me-2'>
                    <i className='fi-facebook'></i>
                  </Button>
                </OverlayTrigger>
                <OverlayTrigger placement='top' overlay={<Tooltip>Share with Twitter</Tooltip>}>
                  <Button href={`https://twitter.com/intent/tweet?url=${encodeURIComponent(currentUrl)}&text=${encodeURIComponent('Check this out!')}`} size='xs' variant='icon btn-light-primary rounded-circle shadow-sm mb-md-2 me-md-0 me-2'>
                    <i className='fi-twitter'></i>
                  </Button>
                </OverlayTrigger>
                <OverlayTrigger placement='top' overlay={<Tooltip>Share with LinkedIn</Tooltip>}>
                  <Button href={`https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(currentUrl)}`} size='xs' variant='icon btn-light-primary rounded-circle shadow-sm mb-md-2 me-md-0 me-2'>
          ...
          <Col xs={12} md={10} lg={8}>
            <div className='mb-4 pb-md-3'>
              <a href='#' className='d-flex align-items-center text-body text-decoration-none'>
                ...
                />
                <div className='ps-3'>
                  <h2 className='h6 mb-1'>Admin</h2>
                  <span className='fs-sm'>Imperial Property Group Agent</span>
                </div>
              </a>
            </div>
            <h6 style={{ textAlign: 'justify' }}>{post.description}</h6>
